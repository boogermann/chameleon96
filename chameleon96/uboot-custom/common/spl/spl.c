...
/* YHN, Forcing CV96 to stay powered up when manual power button is being pressed */
    int reg;
	
	reg = readl(0xFF709004);
	reg |= 0x00008008;
    writel(reg, 0xFF709004);        // Set GPIO44 to output mode, USER LED 4 output
	
	reg = readl(0xFF709000);
	reg |= 0x00008008;
    writel(reg, 0xFF709000);       // Set GPIO44 to high, USER LED 4 is off
	
	puts("\nSetting Power FET to Power On \n");

/* YHN, Force all LED off, USER1 on */
	reg = readl(0xFF708004);
	reg |= 0x02404000;
    writel(reg, 0xFF708004);        // Set USER LED 1, 2 ,3 to output

    reg = readl(0xFF708000);
	reg |= 0x02400000;
	reg &= 0xFFFFBFFF;
    writel(reg, 0xFF708000);        // Set USER LED 1, 2 ,3 to output
