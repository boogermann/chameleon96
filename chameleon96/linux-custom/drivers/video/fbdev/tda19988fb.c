...
//extern const struct fb_videomode cea_mode[64];
const struct fb_videomode cea_mode[64] = { 
 ...
 */
int lcd_fb_mode_is_equal(const struct fb_videomode *mode1,
 ...
 */
int lcd_fb_var_to_vic(struct fb_var_screeninfo *var)
{
	int i;
	struct fb_videomode m;

	printk(KERN_ERR "TDA: %s\n", __FUNCTION__);

	for (i = 0; i < ARRAY_SIZE(cea_mode); i++) {
		fb_var_to_videomode(&m, var);
		if (lcd_fb_mode_is_equal(&m, &cea_mode[i]))
			break;
	}

	if (i == ARRAY_SIZE(cea_mode))
		return 0;
...
static uint8_t
cec_read(struct tda19988_data *priv, uint8_t addr)
{
...
static int reg_read(struct tda19988_data *data, uint16_t reg)
{
	...
	printk(KERN_ERR "TDA: %s read 0x%08x from 0x%08x\n", __FUNCTION__, val, reg);
	if (ret < 0)
		return ret;
	return val;
}
...
static void reg_write(struct tda19988_data *data, uint16_t reg, uint8_t val)
{
	...
	int ret;
	printk(KERN_ERR "TDA: %s writing 0x%08x to 0x%08x\n", __FUNCTION__, val, reg);
...
{
	int old_val;
	printk(KERN_ERR "TDA: %s setting 0x%08x to 0x%08x\n", __FUNCTION__, reg, val);
...
static void reg_clear(struct tda19988_data *data, uint16_t reg, uint8_t val)
{
	...
	printk(KERN_ERR "TDA: %s clearing 0x%08x from 0x%08x \n", __FUNCTION__, val, reg);
	old_val = reg_read(data, reg);
	if (old_val >= 0)
		reg_write(data, reg, old_val & ~val);
}
...
{
        struct delayed_work *det_work = to_delayed_work(work);
        struct tda19988_data *priv =
                        container_of(det_work, struct tda19988_data, det_work);
	printk(KERN_ERR "TDA: %s\n", __FUNCTION__);
...
{
        struct tda19988_data *priv = data;
	...
	printk(KERN_ERR "TDA: %s\n", __FUNCTION__);
        ...
        dev_err(&priv->cec->dev,
                "tda irq sta %02x cec %02x lvl %02x f0 %02x f1 %02x f2 %02x\n",
                sta, cec, lvl, flag0, flag1, flag2);

        if ((flag2 & INT_FLAGS_2_EDID_BLK_RD) && priv->wq_edid_wait) {
		printk(KERN_ERR "TDA: EDID READ\n");
                priv->wq_edid_wait = 0;
                wake_up(&priv->wq_edid);
        } else if (cec != 0) {                  /* HPD change */
		printk(KERN_ERR "TDA: HPD change\n");
                schedule_delayed_work(&priv->det_work, HZ/10);
        }
        return IRQ_HANDLED;
}
...
static void tda19988_fb_var(struct tda19988_data *data, struct fb_var_screeninfo var) {
	...
	cea_mode=lcd_fb_var_to_vic(&var);
		...
		const struct i2c_device_id *id)
{
    ...
    cec_write(data, REG_CEC_ENAMODS, 
	...
	if (rev != 0x0301) {
        ...
        cec_write(data, REG_CEC_FRO_IM_CLK_CTRL,
                        CEC_FRO_IM_CLK_CTRL_GHOST_DIS | CEC_FRO_IM_CLK_CTRL_IMCLK_SEL);

        /* initialize the optional IRQ */

	printk(KERN_ERR "TDA: if client irq %d\n", client->irq);
        if (client->irq) {
                int irqf_trigger;
		printk(KERN_ERR "TDA: Initializing EDID read\n");

                /* init read EDID waitqueue and HDP work */
                init_waitqueue_head(&data->wq_edid);
                INIT_DELAYED_WORK(&data->det_work, tda998x_hpd);

                /* clear pending interrupts */
                reg_read(data, REG_INT_FLAGS_0);
                reg_read(data, REG_INT_FLAGS_1);
                reg_read(data, REG_INT_FLAGS_2);

                irqf_trigger =
                        irqd_get_trigger_type(irq_get_irq_data(client->irq));

                int ret = request_threaded_irq(client->irq, NULL,
                                           tda998x_irq_thread,
                                           irqf_trigger | IRQF_ONESHOT,
                                           "tda998x", data);
                if (ret) {
			printk(KERN_ERR "TDA: Failed requesting IRQ %d (%d)\n", client->irq, ret);
                        dev_err(&client->dev,
                ...
                /* enable HPD irq */
                cec_write(data, REG_CEC_RXSHPDINTENA, CEC_RXSHPDLEV_HPD);
        }

        if (client->irq) {
		printk(KERN_ERR "TDA: enabling EDID read IRQ\n");
       		/* enable EDID read irq: */
       		reg_set(data, REG_INT_FLAGS_2, INT_FLAGS_2_EDID_BLK_RD);

		u8 edid_data[128];
		memset(edid_data, 0x00, 128);
		read_edid_block(data, edid_data, 1, 128);
	}
...
static int read_edid_block(void *data, u8 *buf, unsigned int blk, size_t length)
{
	...
	printk(KERN_ERR "TDA: %s\n", __FUNCTION__);
        ...
        if (priv->hdmi->irq) {
		printk(KERN_ERR "TDA: edid read waiting on IRQ %d\n", priv->hdmi->irq);
		...
		printk(KERN_ERR "TDA: reading edid\n");
                for (i = 100; i > 0; i--) {
                        msleep(1);
