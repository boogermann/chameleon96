...
static DEFINE_MUTEX(csdio_mutex);
...
/*
 * Ioctl wrapper to use "unlocked"
 */
static long csdio_unlocked_transport_ioctl(struct file *file, uint cmd, ulong arg)
{
	struct inode *inode;
	long ret;
	...
	mutex_lock(&csdio_mutex);
	ret = csdio_transport_ioctl(inode, file, cmd, arg);
	mutex_unlock(&csdio_mutex);
	return ret;
}
...
static const struct file_operations csdio_transport_fops = {
	...
	.unlocked_ioctl =    csdio_unlocked_transport_ioctl,
...
static long csdio_unlocked_ctrl_ioctl(struct file *file, uint cmd, ulong arg)
{
	struct inode *inode;
	long ret;
	...
	mutex_lock(&csdio_mutex);
	ret = csdio_ctrl_ioctl(inode, file, cmd, arg);
	mutex_unlock(&csdio_mutex);
	return ret;
}
...
static const struct file_operations csdio_ctrl_fops = {
	.owner =	THIS_MODULE,
	.unlocked_ioctl =	csdio_unlocked_ctrl_ioctl,
